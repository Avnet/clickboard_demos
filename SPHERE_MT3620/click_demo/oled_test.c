...
void testdrawline(OLEDB_SSD1306* ptr) 
{
    ...
    for(i=0; i<AvnetGFX_width(); i+=4) {
        AvnetGFX_drawLine(0, 0, (int16_t)i, AvnetGFX_height()-1, PIXON);
        oledb_display(ptr, DISP_NORMAL); // Update screen with each newly-drawn line
        _delay(100);
        }
    for(i=0; i<AvnetGFX_height(); i+=4) {
        AvnetGFX_drawLine(0, 0, AvnetGFX_width()-1, (int16_t)i, PIXON);
    ...
    for(i=0; (int16_t)i<AvnetGFX_width(); i+=4) {
        AvnetGFX_drawLine(0, AvnetGFX_height()-1, (int16_t)i, 0, PIXON);
        oledb_display(ptr, DISP_NORMAL);
        _delay(100);
        }
    for(i=(int)AvnetGFX_height()-1; i>=0; i-=4) {
