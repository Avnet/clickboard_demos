...
I2C         i2c_handle(I2C_SDA , I2C_SCL );
InterruptIn intPin(D2);  //Slot#1=D2, Slot#2=D3
int         intOccured;

int read_i2c( uint8_t addr, uint16_t count, uint8_t* ptr ) 
{
    return i2c_handle.read( MAX30101_SAD<<1, (char*)&addr, 1);
}
...
void write_i2c( uint8_t addr, uint16_t count, uint8_t* ptr)
{
    ...
    i2c_handle.write(MAX30101_SAD<<1, (const char*)buff, count+1);
...
int main(int argc, char *argv[]) 
{
    ...
    int            run_time = 30*250;  //default to 30 second run time
